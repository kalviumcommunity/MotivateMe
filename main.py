...
import requests
import json
from token_utils import log_tokens_after_call  # existing helper

def call_google_studio(prompt_text: str, api_key: str = None, temperature: float = 0.0, top_k: int | None = None, top_p: float | None = None):
    """
    Calls Google Studio / Gemini model with generationConfig including temperature/top_k/top_p.
    ...
    # Extract text (Gemini-like). Fallback to dumping the whole response.
    try:
        response_text = api_response["candidates"][0]["content"]["parts"][0]["text"]
    except Exception:
        response_text = json.dumps(api_response)

    # Log token usage (provider usage if present or local estimate)
...
# example usage (do NOT hardcode API_KEY in repo)
if __name__ == "__main__":
    prompt = "Provide a motivational quote for mood: 'tired'. Respond in JSON with fields mood, quote, author, suggested_action."
    api_resp, model_text = call_google_studio(prompt_text=prompt, temperature=0.2, top_k=16, top_p=0.9)
